...
Continuity from the input terminal through the protection chain is worth another 30 seconds. On PCB 2 that path is screw terminal, polyfuse, bridge rectifier, TVS — four parts that can each be missing, backwards or cold-jointed.

## First power: watch the current, not the board

Use a bench supply with an adjustable current limit. Set the voltage to the nominal input, then set the limit just above what the board should idle at — for PCB 2 at 24 V, around 100 mA, generous for a buck's quiescent draw plus an idle LDO.
...
- I2C: run a bus scan. On PCB 1 it should return exactly the BME280 at 0x76 or 0x77 depending on the SDO strap, plus the second sensor's address. Nothing found usually means missing pull-ups or the wrong pins; every address responding means SDA is stuck low.
...
- Serial: confirm the USB or UART path and the boot straps. On PCB 1, BOOT and RESET have to actually drop the ESP32-S3 into download mode, and the first flash attempt is that test.
...
On PCB 2 the firmware comes up with the relay and MOSFET outputs unloaded, and each channel gets exercised one at a time before anything real is wired to it. On PCB 1, ESPHome goes on only after the I2C scan has already named both sensors.
